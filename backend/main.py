...
def apply_heuristic(request: ApplyHeuristicRequest):
    ...
    try:
        ...
        # 2. Merge Supporting Fields SAFELY
        # EXCLUDE 'Outsource_Cost' from this list so we don't overwrite the scheduler's calculation with $0 from main DB
        cols_to_merge = ['Total_Proc_Min', 'Release_Time_Min', 'Release_Time', 'Due_Time_Min', 'Transfer_Min', 'Setup_Time']
        sup_cols = [c for c in cols_to_merge if c in state.df_ops.columns]
        
        if sup_cols:
            # Merge with suffix to avoid collisions
            schedule_df = schedule_df.merge(
                state.df_ops[['Operation_ID'] + sup_cols], 
                on='Operation_ID', 
                how='left',
                suffixes=('', '_db')
            )

        # 3. Clean up merged columns
        # If the schedule didn't have the column, use the DB version. If it did, keep schedule version.
        for col in sup_cols:
            db_col = f"{col}_db"
            if db_col in schedule_df.columns:
                if col not in schedule_df.columns or schedule_df[col].isna().all():
                    schedule_df[col] = schedule_df[db_col]
                schedule_df.drop(columns=[db_col], inplace=True)

        # 4. Ensure Calculation Fields Exist (Fill Defaults)
        if 'Proc_Time' not in schedule_df.columns:
             schedule_df['Proc_Time'] = pd.to_numeric(schedule_df.get('Total_Proc_Min', 0), errors='coerce').fillna(0)
        
        if 'Transfer_Time' not in schedule_df.columns:
             schedule_df['Transfer_Time'] = pd.to_numeric(schedule_df.get('Transfer_Min', 0), errors='coerce').fillna(0)

        schedule_df['Setup_Time'] = schedule_df.get('Setup_Time', 0).fillna(0)

        # 5. Handle Release Day Display
        try:
            if 'Release_Time' not in schedule_df.columns:
                schedule_df['Release_Time'] = schedule_df.get('Release_Time_Min', 0)
            
            # Calculate Day (Minutes / 480)
            schedule_df['Release_Day'] = (pd.to_numeric(schedule_df['Release_Time'], errors='coerce') / 480).fillna(0).astype(int)
        except Exception:
            pass
